...
TEST(PointCloudTest, ConcatenateFeatures3D)
{
    ...
    DP lefts(ref3D.features.leftCols(leftPoints), ref3D.featureLabels);
    DP rights(ref3D.features.rightCols(rightPoints), ref3D.featureLabels);
...
TEST(PointCloudTest, ConcatenateDescSame)
{
    ...
    const int leftPoints(ref3D.features.cols() / 2);
    const int rightPoints(ref3D.features.cols() - leftPoints);
    DP lefts(ref3D.features.leftCols(leftPoints), ref3D.featureLabels, PM::Matrix::Random(5, leftPoints), Labels(Label("Desc5D", 5)));
    DP rights(ref3D.features.rightCols(rightPoints), ref3D.featureLabels, PM::Matrix::Random(5, rightPoints), Labels(Label("Desc5D", 5)));
    lefts.concatenate(rights);
...
TEST(PointCloudTest, ConcatenateDescSame2)
{
    ...
    DP lefts(ref3DCopy.features.leftCols(leftPoints),
             ref3DCopy.featureLabels,
             ref3DCopy.descriptors.leftCols(leftPoints),
             ref3DCopy.descriptorLabels);
    DP rights(ref3DCopy.features.rightCols(rightPoints),
              ref3DCopy.featureLabels,
...
TEST(PointCloudTest, ConcatenateDescDiffName)
{
    ...
    const int leftPoints(ref3D.features.cols() / 2);
    const int rightPoints(ref3D.features.cols() - leftPoints);
    DP lefts(ref3D.features.leftCols(leftPoints), ref3D.featureLabels, PM::Matrix::Random(5, leftPoints), Labels(Label("MyDesc5D", 5)));
    DP rights(
        ref3D.features.rightCols(rightPoints), ref3D.featureLabels, PM::Matrix::Random(5, rightPoints), Labels(Label("YourDesc5D", 5)));
    lefts.concatenate(rights);
...
TEST(PointCloudTest, ConcatenateDescDiffSize)
{
    ...
    const int leftPoints(ref3D.features.cols() / 2);
    const int rightPoints(ref3D.features.cols() - leftPoints);
    DP lefts(ref3D.features.leftCols(leftPoints), ref3D.featureLabels, PM::Matrix::Random(3, leftPoints), Labels(Label("DescND", 3)));
    DP rights(ref3D.features.rightCols(rightPoints), ref3D.featureLabels, PM::Matrix::Random(5, rightPoints), Labels(Label("DescND", 5)));
...
TEST(PointCloudTest, AssertConsistency)
{
    ...
    EXPECT_NO_THROW(ref3DCopy.assertDescriptorConsistency());
    ...
    ref3DCopy.descriptorLabels = labels;
    EXPECT_THROW(ref3DCopy.assertDescriptorConsistency(), std::runtime_error);


    // We add a descriptor with the wrong number of points
    PM::Matrix descriptors = PM::Matrix::Random(2, 10);
    ref3DCopy.descriptors = descriptors;

    EXPECT_THROW(ref3DCopy.assertDescriptorConsistency(), std::runtime_error);

    // We add a descriptor with the wrong dimension
    descriptors = PM::Matrix::Random(1, ref3DCopy.getNbPoints());
    ref3DCopy.descriptors = descriptors;
    EXPECT_THROW(ref3DCopy.assertDescriptorConsistency(), std::runtime_error);
}
