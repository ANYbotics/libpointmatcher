...
public:
    ...
    {
        testedOutlierFilter = PM::get().OutlierFilterRegistrar.create(name, params);
...
{
    addFilter("MaxDistOutlierFilter", { { "maxDist", toParam(1.0) } });
...
TEST_F(OutlierFilterTest, MinDistOutlierFilter3D)
{
    ...
    extraOutlierFilter = PM::get().OutlierFilterRegistrar.create("MaxDistOutlierFilter", { { "maxDist", toParam(1.0) } });
...
{
    addFilter("VarTrimmedDistOutlierFilter", { { "minRatio", toParam(0.60) }, { "maxRatio", toParam(0.80) }, { "lambda", toParam(0.9) } });
    validate3dTransformation();
}

OutlierFiltersImpl<NumericType>::OutlierWeights VarTrimLambdaTest(const NumericType lambda)
{
    OutlierFiltersImpl<NumericType>::VarTrimmedDistOutlierFilter filter(
        { { "minRatio", toParam(0.0000001) }, { "maxRatio", toParam(1.0) }, { "lambda", toParam(lambda) } });
